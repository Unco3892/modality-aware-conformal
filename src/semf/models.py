...
class MultiXGBs(MultiModelBase):
    ...
    def __init__(self, **kwargs):
        ...
        # None falls through to the XGBoost library default for that parameter.
        self.xgb_extra_params = {name: kwargs.get(key) for key, name in
                                 (('xgb_learning_rate', 'learning_rate'),
                                  ('xgb_subsample', 'subsample'),
                                  ('xgb_colsample_bytree', 'colsample_bytree'))
                                 if kwargs.get(key) is not None}
        print(f"       ** Number of estimators: {self.n_estimators}, Max depth: {self.xgb_max_depth}, Early stopping patience: {self.xgb_patience}, Validation split: {self.models_val_split}, Extra params: {self.xgb_extra_params or 'library defaults'}")
