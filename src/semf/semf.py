...
class SEMF:
    ...
    Args:
        ...
        weight_alignment (str, optional): How the ``(observation, replication)``
            weight matrix is flattened for the replication-major M-step rows.
            ``"aligned"`` pairs row ``r * n + i`` with ``w_R[i, r]``;
            ``"legacy"`` preserves the historical C-order implementation.
            Default is ``"aligned"``; legacy behavior must be requested
            explicitly for artifact reproduction.
        allow_partial_recovery (bool, optional): If ``True``, return an
            explicitly incomplete model after an iteration raises. The default
            is ``False`` and re-raises, which is required for paper-producing
            batch jobs.
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert isinstance(allow_partial_recovery, bool), (
            "allow_partial_recovery must be a boolean"
        )
        if weight_alignment not in {"legacy", "aligned"}:
            raise ValueError(
                "weight_alignment must be either 'legacy' or 'aligned', "
                f"got {weight_alignment!r}"
            )
        ...
        # None means the XGBoost library default, so omitting these keys keeps
        # the historical behaviour of every caller that never set them.
        self.xgb_learning_rate = tree_config.get('xgb_learning_rate', None) if tree_config else None
        self.xgb_subsample = tree_config.get('xgb_subsample', None) if tree_config else None
        self.xgb_colsample_bytree = tree_config.get('xgb_colsample_bytree', None) if tree_config else None
    ...
    def compute_weights(self):
        ...
        if self.i == 1 or self.use_constant_weights:
            ...
            self.w_comp = self._flatten_replication_weights(
                self.w_R, alignment=self.weight_alignment
            )
        ...
        if np.mean(self.w_comp) < epsilon or np.max(self.w_comp) == 0:
            if self.i == 1:
                ...
                self.w_comp = self._flatten_replication_weights(
                    np.full_like(self.w_R, 1.0 / self.R),
    ...
    @staticmethod
    def _flatten_replication_weights(w_R, alignment="aligned"):
        """Flatten per-observation replication weights for M-step row order.

        ``x_comp``, ``y_comp``, and ``flatten_3d_array`` all produce
        replication-major rows:

        ``(r=0, i=0..n-1), (r=1, i=0..n-1), ...``.

        For ``w_R[i, r]``, the corresponding aligned vector is therefore the
        column-major flattening of ``w_R``.  The legacy C-order option is kept
        only so historical artifacts can still be reproduced explicitly.
        """
        w_R = np.asarray(w_R)
        if w_R.ndim != 2:
            raise ValueError(f"w_R must be two-dimensional, got shape {w_R.shape}")
        if alignment == "aligned":
            return w_R.flatten(order="F")
        if alignment == "legacy":
            return w_R.flatten(order="C")
        raise ValueError(
            "alignment must be either 'legacy' or 'aligned', "
            f"got {alignment!r}"
        )
    ...
    def train_semf(self):
        ...
        Training is fail-fast by default. Set ``allow_partial_recovery=True``
        only for interactive diagnosis; such a model is marked with
        ``training_complete=False`` and must not be used for paper artifacts.
        """
        self.training_complete = False
        self.training_error = None
        self.last_successful_iteration = 0
        ...
        try:
            while self.continue_:
                ...
                # An iteration counts as successful only after all model,
                # variance, inference, and diagnostic steps have completed.
                self.last_successful_iteration = self.i
                ...
                if self.stopping_rule():
                    ...
                    elif getattr(self, "stopped_at_max_it", False):
                        print("Stopping: maximum outer iterations reached; "
                              "returning the final iteration's models")
                    else:
            ...
            self.training_error = repr(e)
            if self.allow_partial_recovery:
                print(f"Encountered error during training iteration: {e}")
                print(
                    "Returning an explicitly incomplete model from iteration "
                    f"{self.last_successful_iteration}; do not use it for paper artifacts."
                )
                return self
            raise RuntimeError(
                f"SEMF training failed in iteration {self.i}; "
                "partial recovery is disabled"
            ) from e
        self.training_complete = True
        return self
